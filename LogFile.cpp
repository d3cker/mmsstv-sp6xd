...
//		if( !m_fhd.size ) unlink(m_FileName.c_str());	// 0件の時は消去
		m_Open = 0;
	}
	m_EditFlag = 0;
	m_Index.ClearIndex();
...
int CLogFile::Open(LPCSTR pName, BOOL fNew)
{
	...
	int omode = O_RDWR|O_BINARY;		// 初回は新規作成なしでオープン
	int n = 0;
	while(1){
		if( (m_Handle = open(pName, omode, S_IREAD|S_IWRITE)) > 0 ){
			...
			else if( omode & O_CREAT ){		// 新規ファイル　2回目のパスのみ
				InitHeader();
		...
		// オープンエラーの場合
		if( !(omode & O_CREAT) && (errno == ENOENT) ){	// １回目でファイルが存在しない場合
			n++;
            if( n >= 2 ){
			    if( fNew ){
					if( YesNoMB(MsgEng ? "Log file (%s) does not exist, create it ?" : "ログファイル(%s)が見つかりません. 新規に作成しますか ?", pName) != IDYES ) return FALSE;
   				}
        		omode |= O_CREAT;		// 2回目は新規作成付きでオープン
            }
            else {
				::Sleep(1000);
            }
